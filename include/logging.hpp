...
public:
	...
	struct c_log_base
	{
		...
		template<class... Args> c_log& operator()(const Args&... args) const { return begin().put<Args...>(args...).put(Endl); }
	...
	static constexpr struct c_log_done : public c_log_base {
		c_log& begin() const override { return instance().put(Green, " - [DONE]", Reset); }
	} Done{};

	static constexpr struct c_log_debug : public c_log_base {
		c_log& begin() const override { return instance().put(Cyan, " - [DEBUG]", Reset); }
	} Debug{};

	static constexpr struct c_log_error : public c_log_base {
		c_log& begin() const override { return instance().put(Red, " - [ERROR]", Reset); }
	} Error{};

	static constexpr struct c_log_info : public c_log_base {
	private:
		c_log& begin() const override { return instance().put(Yellow, " - [INFO]", Reset); }
	} Info{};
